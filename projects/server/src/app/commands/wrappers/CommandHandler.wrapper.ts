...
import { EntityWrapper, EntityWrapperCommand } from '@commands/interfaces/EntityWrapper';
import { StringUtils } from '@commands/utils/StringUtils';

const buildHelpPage = (path: string[], wrapper: EntityWrapper<unknown>): string => {
  const entries = Array.from(wrapper.commands.entries());
  const commandList = entries.map(([cmdName, cmd]) => {
    const params = cmd.args?.length ? `(${cmd.args.join(', ')})` : '';
    return `${cmdName}${params}`;
  });
  return StringUtils.buildPage([
    { title: path.join(':') || 'root', text: wrapper.info ?? 'no info available' },
    { title: 'Commands', text: StringUtils.buildList(commandList) },
  ]);
};

export const CommandHandlerWrapper: EntityWrapper<CommandHandler> = {
  info: 'Used to manage commands',
  commands: new Map<string, EntityWrapperCommand<CommandHandler>>([
    [
      'help',
      {
        ...
        fn: (commandHandler, pathStr) => {
          ...
          const resolved = commandHandler.resolve(segments);
          if (resolved) {
            return buildHelpPage(segments, resolved.wrapper);
          }
          ...
          if (segments.length > 1) {
            ...
            const parentResolved = commandHandler.resolve(parentPath);
            if (parentResolved) {
              const command = parentResolved.wrapper.commands.get(commandName);
              if (command) return command.info ?? 'no info available';
            }
          }
