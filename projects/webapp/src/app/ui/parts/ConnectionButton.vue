<template>
  <ButtonSections class="connection-button">
    <Button
      ...
      :icon="connectionIcon"
      @click.stop="wsService.connectToPort(wsService.port.value)"
    />
    <Button
      :class="connectionClass"
      :text="connectionText"
...
const connectionClass = computed(() => ({
  success: wsService.status.value === WsConnectionStatus.Connected,
  warning: wsService.status.value === WsConnectionStatus.Connecting,
  danger: wsService.status.value === WsConnectionStatus.Disconnected,
}));
...
const connectionItems = computed<readonly MenuItemData[]>(() => [
  {
    name:
      wsService.status.value === WsConnectionStatus.Disconnected
        ? `Connect to ${wsService.label}`
        : `Reconnect to ${wsService.label}`,
    icon: 'radio',
    action: () => wsService.connectToPort(wsService.port.value),
  },
  {
    name: 'Disconnect',
    icon: 'radio-tower',
  ...
  ...PORT_OPTIONS.map((port) => ({
    name: `localhost:${port}`,
    icon: 'server',
    endIcon: port === wsService.port.value ? 'check' : undefined,
    action: () => wsService.connectToPort(port),
  })),
  {
    name: 'Custom Port',
    icon: 'settings',
    action: () => {
      const value = window.prompt('Port', String(wsService.port.value));
      if (!value) return 'dismiss';

      const port = Number(value);
      if (!Number.isInteger(port) || port < 1 || port > 65535) return 'dismiss';
